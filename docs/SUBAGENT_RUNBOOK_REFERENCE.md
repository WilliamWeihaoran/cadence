# Long Subagent Runbook Reference

The incident narratives and task-specific procedures lifted out of `docs/SUBAGENT_RUNBOOK.md` by
T-1333, kept whole and in their original wording. The runbook itself is now the short mandatory
section an agent reads before starting; this is what it routes to.

**Do not load this whole file by default.** It is 700-odd lines and it exists to be searched. Open
the one section the runbook's routing table sends you to, or `rg` for the symptom you are looking
at. Every rule stated in the runbook is also here, in the incident that produced it — the runbook
says *what you must do*, this says *what happened to the agent who did not*, which is the half you
want when a rule surprises you and the half you never want before starting work.

Nothing here was rewritten to fit. The T-1333 split moved prose; it did not edit it. Where a
section says "the runbook already says", it means the document as it stood before the split, and
the rule it refers to is in the front matter now.

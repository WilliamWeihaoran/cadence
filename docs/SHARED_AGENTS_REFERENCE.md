...
This is the former long `Cadence/Shared/AGENTS.md`, preserved so no shared-surface detail is lost
while the active scoped guide stays compact. Do not load this whole file by default; search it for
the specific rule or incident you need.
...
component is a second declaration inside another component's file, say so in this guide's inventory
or the relevant scoped guide. The compact `CLAUDE.md` deliberately does not carry a full file
inventory any more; the old long inventory is preserved in `docs/CLAUDE_REFERENCE.md` for lookup.

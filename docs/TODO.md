...
- [T-1095] **CLOSED 2026-09-11 (agent `mcpwrite3`) — the MCP write surface can now change a
  ...
  adds, renames, recolours, redates, archives, completes and reorders the kanban columns of an
  existing area or project, and answers the same `CadenceContainerSummary` `get_container_summary`
  does. **No model change**; sections are still `TaskSectionConfig` JSON on the container.
  `serverVersion` and `main.swift`'s handshake are both **0.8.0**, on 0.7.0's own argument: the
  version is the only thing a client can ask about a capability.
  ...
  **Scope, and why the other two legs are not in it.** (b) — constructors for goal, habit, tag,
  ...
  the leg the ticket predicts "a second seeding job will hit first", and it is the one that
  actually needed the boundary decision.

  **The decision T-1095 asked for was taken, and the reason it gives for it is wrong.** Three
  `Cadence/Shared/` files joined `CadenceMCPServer`'s explicit Sources phase:
  `CadenceSectionConfigMerge`, `CadencePendingChangePersistence` and `CadenceSectionEditingSupport`.
  The ticket predicted the first, on the grounds that mutating an existing list "genuinely needs
  `base`/`edited`/`current`". **It does not.** This call reads the columns and writes them inside
  one synchronous frame, so `base == current` and the merge degenerates to "apply this edit" —
  exactly the case `mutateSectionConfigs`' own comment names and exactly the reason [[T-799]] kept
  the file out for the create path. A caller cannot supply a real `base` either:
  `CadenceSectionSummary` has never carried a column `uuid`, so an MCP caller addresses a column by
  **name** and holds no snapshot this surface could reconcile. What actually earns the coupling:
  - **`CadenceSectionEditingSupport.applySectionNameChanges` is load-bearing and was not predicted
    at all.** `AppTask.sectionName` is a plain string; nothing re-points a card when its column is
    renamed. Without it a rename here strands every card on a name no column has, and
    `CadenceReadService.sectionSummaries` answers the orphan back as a **phantom column** — so an
    assertion that "Doing exists" would pass over the bug. That is [[T-1053]]'s defect, one surface
    along, and it is not a thing a hand-rolled column editor at this boundary would have remembered.
  - **`mutateSectionConfigs` carries [[T-915]]'s guard**: it compares what the *setter would store*
    against what is stored, so a write whose only effect the normaliser discards does not
    re-serialise `sectionConfigsRaw` and push a CloudKit record. This process writes the store the
    running app has open.
  - **`CadencePendingChangePersistence.commitEdit` is the undo this side of the boundary never
    had.** See below.

  **The MCP-side equivalent of the `try? save()` rule, stated.** `AGENTS.md` requires a write to
  commit through `CadencePendingChangePersistence`, `throws`, take `commit:`, and name the failure
  on screen. There is no screen here. The equivalent of "name the failure" is the thrown error the
  router renders as an `isError` tool response, and this surface always had that half —
  `saveNotifyAndAudit` did `try context.save()` and let it propagate. **What it did not have is the
  undo**, and the absence was not cosmetic: `CadenceWriteService` holds one long-lived
  `ModelContext`, so a refused `save()` left the mutation *pending* for the next tool call's
  `save()` to commit — a write the caller was told had failed, landing later, from a call that never
  mentioned it. `saveNotifyAndAudit` now takes `undo:` and commits through `commitEdit`, and
  `CadenceWriteService` gained the `commit:` seam every other commit site in this repository has,
  because a `save()` that throws cannot be provoked out of an in-memory container and an undo path
  no test can reach is an undo path no test can prove. **Every other write arm still inserts with
  no undo** — `commitInsert` is now compiled into this target and the sweep is [[T-1121]], not a
  rider on a column editor.

  Four more decisions worth keeping:
  - **Validate everything, then write once.** `plannedSectionConfigs` builds the whole resulting
    array out of a pure function and throws before the model is touched, so a refusal in the reorder
    leg cannot leave a rename half-applied in the shared context. The per-operation helpers
    (`addSectionConfig`, `updateSectionConfig`) each write separately and could not give that.
  - **Refusals where the list forgives**, which is [[T-799]]'s argument one door along. The
    container's normaliser silently drops a blank name, a case-insensitive duplicate, and
    `isCompleted`/`isArchived` on `Default`; all of them `throw` here, because a caller who only
    ever reads the word "success" cannot notice a dropped argument. Renaming `Default` is refused
    for a stronger reason than taste: the normaliser re-synthesises it under that name on the next
    read, so the rename would leave **two** columns. A request that changes nothing (the colour it
    already has) answers `noChanges` rather than success.
  - **`columnOrder` must be a complete permutation and must start with `Default`.** A partial order
    is refused rather than guessed at, and `Default` is pinned because the normaliser moves it to
    index 0 on every write — any other position the caller asked for would not be what got stored.
  - **There is no `removeColumns`.** Archiving is the reversible equivalent and is offered;
    removing a column destroys its colour, due date and flags with no undo and no confirmation.
    That is the deletion decision this ticket asks be taken separately — see [[T-1120]].

  **Found on the way, fixed here:** `plugins/cadence-mcp/scripts/run-cadence-mcp.sh` decided whether
  its warm binary was stale by watching **three** files under `Cadence/Services` and `Cadence/Shared`
  while the target compiles about fourteen. Editing `TagSupport`, `NoteMigrationService`,
  `CadenceStoreSupport`, `CadenceTaskRecurrenceWorkflowSupport` or any of the rest left the binary
  stale and the next smoke run measuring the *previous* build. It now watches both directories
  whole; over-rebuilding costs time, under-rebuilding costs a measurement nobody can tell is wrong.

  **MEASURED, on a `git archive HEAD` tree carrying the change (`924fab8`):** `CadenceMCPServer`
  scheme built separately into private DerivedData — `** BUILD SUCCEEDED **`, **0 errors, 0
  warnings** (`grep -cE '\.swift:[0-9]+:[0-9]+: error:'` = 0, `grep -c 'warning:'` = 0), log
  confirms it recompiled `CadenceWriteService.swift` and all three newly added `Shared/` files.
  `plugins/cadence-mcp/scripts/smoke-test.py` green: `OK cadence-mcp 0.8.0`, `OK tools/list 33
  tools`, `OK dispatched 33/33 advertised tools`, 36 audit entries — and its new block drives six
  refusals, a rename whose card is checked to have followed, an add-plus-reorder and an archive over
  stdio against the board T-799 seeds. App-scheme `-only-testing:CadenceTests` counts are in the
  commit message. Six new `@Test`s in `CadenceWriteServiceTests`, including the refused-commit undo.
...
- [T-1122] **The MCP write surface can create a context, a list and a task, and none of the other
  ...
  execution, because `plugins/cadence-mcp/scripts/smoke-test.py` drives a fresh fixture store and
  cannot make a row of any of them — so `list_task_bundles`, `list_goals`, `list_habits` and
  `list_links` are dispatched and assert `[]` (see T-269). Six write paths, six DTO decisions and
  six sets of refusals; `Tag` and `Goal` both have existing helpers the write service must go
  through (`TagSupport`, and `ModelContext.attachList` for `GoalListLink` — never a hand-rolled
...
- [T-1071] **CLOSED 2026-09-07 (agent `swallow`) — the notepad `+` names a refused note instead of doing nothing.** `iOSNotesView.createNotepadNote` is a `do`/`catch` that sets `createFailureNotice` from the shared `editFailureNotice` sentence and opens the note only past the `catch`; the notice draws above the index column, where `iOSListNotesView` puts its own. `NoteMigrationService.createPermanentNote` un-inserts the note when the store refuses it, rather than leaving a bare `try context.save()` — without that the new sentence ("Nothing was changed") would have been false, since the refused `Note` stayed pending in the app's one `ModelContext` for the next unrelated save to take. **The undo is spelled out there rather than routed through `CadencePendingChangePersistence.commitInsert`, and measurement is why:** that file is in `CadenceMCPServer`'s explicit Sources phase and the shared helper is not, so the first attempt turned `CadenceTargetSourceMembershipTests.mcpServerSourcesOnlyReferenceTypesThatTargetCompiles` red — the `aaa0064` shape, caught by the guard built for it. Pinned by `CadenceTagAndNoteCommitSurfaceTests.theNotepadPlusNamesARefusedNoteRatherThanDoingNothing`. **The folder-sheet half of this entry was split out as [[T-1093]]**, because the write underneath it has four production call sites across both platforms and one of them is the archive importer, which must not commit per note. **Filed as:** **the notepad `+` fails silently with no notice.**
...
- [T-949] **CLOSED 2026-09-11 (`defaultsroute`) — the same ticket as [[T-745]], and closed as that rather than as a finding of its own.** The two were filed separately and describe one situation: this is T-745's audit re-counted, and the work landed under T-745. Its **residue is fixed by that work**, not deferred — two `CadenceUITests` agents on one simulator can no longer leak a sort order, the hidden-calendar list or the notifications toggle past each other, because all 27 app-only sites are behind `CadenceDefaults.store` now and that is precisely what `-CadenceSuiteName` redirects. Two corrections to the text below, both measured against `a92b659`. **The census reproduces exactly**: 19 non-comment `UserDefaults.standard` references outside `CadenceDefaults.swift`, in the eight files named, and the conclusion drawn from them is right and is kept — none diverges in any shipping configuration. **But 19 is not the exposure, because the census greps one spelling.** It misses 12 parameters spelled `defaults: UserDefaults = .standard` (`PersistenceController` x6, plus `PursuitToGoalMigration`, `AISettingsManager`, `AppleAccountManager`, `CadenceCalendarLinkObservations`, `CadenceNotesEditorPreferences`), and 2 `UserDefaults(suiteName: appGroupIdentifier)` sites in `Theme` and `CadenceWidgetRefreshCenter` that are a **third** store neither filing mentions at all. 33 unrouted sites, not 19. **Filed as:** **`CadenceDefaults` only routes `@AppStorage` and the calendar memory; a service reading
...
- [T-1113] **CLOSED 2026-09-07 (agent `firstrun`) — a fresh Mac install can make its first list without inventing a context to hang the button on.** Filed and closed together, from the external audit at `docs/audits/2026-09-05/first-run-creation.md` (FR-1), which `audittriage`'s T-1096..T-1108 sweep did not reach. **The mechanism, verified at `4ad2178` and unchanged at `afaeb3a`:** `SidebarView.listSections` derives every section from an existing context or an existing list, so zero contexts + zero areas + zero projects produced zero sections and the `ForEach` rendered nothing — and the context header it did not render is the thing carrying the `+` that opens `CreateListSheet`, which has **exactly one call site on macOS**. The file's own doc comment already said so ("the only way to make a list in a given context on macOS"); what nobody had noticed is that on a blank store there is no header for it to be the only way *from*. The route out was Settings → Contexts → New Context, purely to manufacture a header. An App Store reviewer opens a blank store.
  **Not [[T-559]] again.** That ticket made `CreateListSheet(context:)` optional and gave the catch-all header a `+`; neither makes a header *exist* on an empty store. Nor is it "macOS cannot create context-less lists": an existing or synced context-less list produces the "Other" header, and that header works. It is specifically the empty-store case.
...
- [T-1096] **CLOSED 2026-09-07 (agent `rescue-rightclick`) — one right-click overlay that reads the point it was handed.** From `docs/audits/2026-09-05/appkit-behavior.md` (AK-1); landed in `8117017`. **The mechanism:** `RightClickActionView.hitTest(_:)` named its `point` argument and never read it, so *any* `.rightMouseDown` anywhere in the window got this view back — the overlay claimed the event wherever the pointer was. It now delegates to a testable `hitTest(_:currentEvent:)` requiring **both** halves: `super.hitTest(point)` for the spatial one — the argument arrives in the **superview's** coordinate system, so a `bounds.contains` would have tested the right point against the wrong rectangle, and the superclass also folds in `isHidden` and a zero-size frame — and `currentEvent.type == .rightMouseDown` for the event one, which still matters because a primary click must fall through to the SwiftUI button underneath.
  **A byte-identical copy of the type carried a byte-identical copy of the defect.** `Cadence/macOS/Views/SidebarSupportViews.swift` declared its own private `SidebarRightClickEditTrigger` / `RightClickEditView`; it is deleted, and that call site now uses the shared `RightClickActionTrigger` as the other four (`TasksPanelComponents`, `TimelineTaskBlock`, `CalendarPageMonthSupportViews`, `KanbanCardView`) already did.
  Pinned by nine `@Test`s in `CadenceTests/CadenceRightClickTriggerHitTestTests.swift` (`@MainActor`): a point inside is claimed; below, above and beside are refused; hidden and zero-size are refused; a `.leftMouseDown` inside still passes through; a nil event claims nothing; and a source assertion that the sidebar no longer declares its own copy.
  **What these tests do not claim.** The audit marked AK-1 *inferred*, and it stays partly so. They settle the framework-contract half — a point outside the view is refused — and deliberately do **not** reproduce a wrong-row click in a live SwiftUI hosting hierarchy.
  Measured: `-only-testing:CadenceTests` green at 4670 tests in 395 suites, 0 failures, 0 compiler warnings, 0 compile errors, with each edited file's own `SwiftCompile` line present so the warning count is not vacuous. The new suite's first run also printed **5 Main Thread Checker violations** — `NSView(frame:)`, `addSubview:`, `setHidden:`, `setFrame:`, `hitTest:` called off the main thread, invisible in a pass/fail count; `@MainActor` on the suite took them to zero.

- [T-1098] **CLOSED 2026-09-07 (agent `rescue-rightclick`) — the App Store description stops promising settings sync.** From `docs/audits/2026-09-06/request-follow-up.md`; landed in `8117017`. The iCloud Sync paragraph in `docs/app-store-submission-packet.md` dropped ", and settings"; it now reads "Your tasks, notes, habits, and goals sync through your own private iCloud account across your Macs."

...
Push notifications:
- Cadence uses push notifications for exactly one purpose: CloudKit's silent sync notifications. The app ships the `com.apple.developer.aps-environment` entitlement (`development` in Debug, `production` in Release) and registers for remote notifications at launch on both macOS and iOS, so CloudKit can tell the app that the user's private iCloud database changed and app data should be refreshed. Cadence builds macOS and iOS/iPadOS from one app target, and the push entitlement is the one capability whose key is spelled differently per platform, so the target signs from two entitlements files: `Cadence/Cadence.entitlements` carries `com.apple.developer.aps-environment` for macOS and `Cadence/Cadence-iOS.entitlements` carries the bare `aps-environment` for iOS and iPadOS, selected by `CODE_SIGN_ENTITLEMENTS[sdk=iphone*]`. Every other key in the two files is identical, and a test fails if they diverge. Both platforms sync through the same private database. The shared `Cadence/Info.plist` declares the `remote-notification` background mode, which is what lets one of those silent pushes reach the app on iOS; macOS has no such mechanism and ignores the key, as it already ignores the three other UIKit-only keys in that file.

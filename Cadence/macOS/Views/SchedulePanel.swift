...
private struct TaskInspectorDateControl: View {
    let label: String
    let icon: String
...
// MARK: - Quick Create Choice Popover (Time Block or Calendar Event)

struct QuickCreateChoicePopover: View {
    let startMin: Int
    let endMin: Int
    let onCreateTask: (String) -> Void
    let onCreateEvent: ((String, String) -> Void)?   // (title, calendarID)
    let onCancel: () -> Void
...
struct TaskDetailPopover: View {
    ...
    private func minutesLabel(_ m: Int) -> String {
        ...
        return String(format: "%.1f h", Double(m) / 60.0)
    }
...
private struct InspectorPickerHover: ViewModifier {
...
// MARK: - Minutes Field (editable integer field for actual/estimated minutes)

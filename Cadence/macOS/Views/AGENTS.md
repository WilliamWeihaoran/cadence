...
- `CalendarBoard*`, `CalendarPageBoardSupportViews` - the Calendar page's Board mode: day columns plus the pinned Overdue/Unscheduled rails that replaced the Planning page. Rails, drop targets, and window math live in `Shared/CadenceCalendarPlanningSupport.swift`; the board's **widths** — the day column, the rail's two forms, the insets, and the 846pt gate that collapses the rails to strips — are `CadenceCalendarBoardLayout` in `Shared/CadenceRegularPaneLayout.swift` (T-251), which is also where the four constants `KanbanBoardSupport.swift` used to declare now live. A rail is never dropped; see that type's doc for the measurement behind that.

...
enum CadenceScheduleSupport {
    ...
    /// 05:00 matched no row *and* was untimed (`scheduledStartMin == -1`), which at the time meant
    /// the "Ready to Schedule" stack above the grid — it vanished from the pane. Now that the rows
    /// are the whole day every real minute-of-day has a row of its own and nothing is clamped, and
    /// T-1273 removed that stack, so an untimed task's home is the task column, not this pane.
    ///
